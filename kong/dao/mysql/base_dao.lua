-- Kong's mysql base DAO entity. Provides basic functionalities on top of
-- lua-resty-mysql (https://github.com/jbochi/lua-resty-mysql)
...
local mysql_constants = mysql.constants
...
-- @param `session` mysql session to close
-- @return `error`  Error if any
...
-- Build the array of arguments to pass to lua-resty-mysql :execute method.
-- Note:
...
--   into account special mysql values (uuid, timestamps, NULL).
...
-- @return `args`      An ordered list of values to be binded to lua-resty-mysql :execute
-- @return `error`     Error mysql type validation errors
local function encode_mysql_args(schema, t, args_keys)
  local args_to_bind = {}
  local errors
...
-- @param `args`      (Optional) Arguments to the query, simply passed to lua-resty-mysql's :execute()
-- @param `options`   (Optional) Options to give to lua-resty-mysql's :execute()
...
-- @param `options`      Options to pass to lua-resty-mysql :execute()
-- @return :_execute()
function BaseDao:execute(query, columns, args_to_bind, options)
  ...
  if columns and args_to_bind then
    local errors
    args, errors = encode_mysql_args(self._schema, args_to_bind, columns)
    if errors then
...
-- @param  `query`     The query to prepare
-- @return `statement` The prepared statement, ready to be used by lua-resty-mysql.
...
-- @param `paging_state` Start page from given offset. See lua-resty-mysql's :execute() option.
-- @return `res`
-- @return `err`
...
-- @see mysql/factory.lua ':load_daos()'
