-- Kong's mysql Factory DAO. Entry-point for retrieving DAO objects that allow
...
local mysqlFactory = Object:extend()

-- Shorthand for accessing one of the underlying DAOs
function mysqlFactory:__index(key)
  if key ~= "daos" and self.daos and self.daos[key] then
    return self.daos[key]
  else
    return mysqlFactory[key]
  end
end

-- Instantiate a mysql Factory and all its DAOs for various entities
-- @param `properties` mysql properties
function mysqlFactory:new(properties, plugins)
  ...
  for _, entity in ipairs({"apis", "consumers", "plugins"}) do
    self:load_daos(require("kong.dao.mysql."..entity))
  end
...
function mysqlFactory:load_plugins(plugins)
...
function mysqlFactory:load_daos(plugin_daos)
  local dao
...
function mysqlFactory:get_session_options()
  local options = {
  ...
  if self._properties.user and self._properties.password then
    local PasswordAuthenticator = require "mysql.authenticators.PasswordAuthenticator"
